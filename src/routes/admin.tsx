...
import { Plus, Trash2, Upload, X, Save, RotateCcw, GripVertical, MessageSquare, LayoutGrid } from "lucide-react";
    ...
    <main className="max-w-6xl mx-auto px-6 lg:px-10 pt-12 pb-24">
      ...
      <div className="flex gap-1 mb-8 border-b border-border">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-3 text-sm font-medium border-b-2 transition ${tab === "properties" ? "border-accent text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}
        >
          <LayoutGrid className="h-4 w-4" /> Kelola Properti
        </button>
        <button
          onClick={() => setTab("messages")}
          className={`flex items-center gap-2 px-5 py-3 text-sm font-medium border-b-2 transition ${tab === "messages" ? "border-accent text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}
        >
          <MessageSquare className="h-4 w-4" /> Pesan Masuk
        </button>
      </div>

      {/* MESSAGES TAB */}
      {tab === "messages" && <MessagePanel />}
              ...
              <div key={p.id} className="rounded-2xl border border-border bg-card p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-5">
                  ...
                  <Field label="Fasilitas (pisahkan dengan koma)">
                    <input className="input" defaultValue={p.highlights.join(", ")} onBlur={(e) => { const val = e.target.value; update(p.id, { highlights: val.split(",").map((s) => s.trim()).filter(Boolean) }); }} />
                  </Field>
                </div>

                <Field label="Deskripsi">
                  <textarea className="input min-h-[100px]" value={p.description} onChange={(e) => update(p.id, { description: e.target.value })} />
                </Field>
                ...
                <div className="mt-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <label className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-secondary text-secondary-foreground text-xs cursor-pointer hover:opacity-90">
                      ...
                      <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => { onUpload(p.id, e.target.files); e.target.value = ""; }} />
                    </label>
                  </div>
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                      {p.images.map((src, idx) => (
                        <div key={idx} className="relative group aspect-square rounded-lg overflow-hidden bg-muted">
                          ...
                          {idx === 0 && (
                            <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded bg-accent text-accent-foreground text-[10px] font-medium">Cover</div>
                          )}
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100">
                            <button onClick={() => moveImage(p.id, idx, -1)} className="h-7 w-7 rounded-full bg-background grid place-items-center text-xs" title="Geser kiri">←</button>
                            <button onClick={() => moveImage(p.id, idx, 1)} className="h-7 w-7 rounded-full bg-background grid place-items-center text-xs" title="Geser kanan">→</button>
                            <button onClick={() => removeImage(p.id, idx)} className="h-7 w-7 rounded-full bg-destructive text-destructive-foreground grid place-items-center" title="Hapus"><X className="h-3.5 w-3.5" /></button>
                          </div>
                        </div>
                      ))}
                    </div>
